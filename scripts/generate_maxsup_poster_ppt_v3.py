...
OUT_PATH = Path("/Users/jayden/Desktop/7404 comp/project/MaxSup_A0_HiFi_Poster_v3.pptx")
LOGO_PATH = Path("/Users/jayden/Desktop/7404 comp/project/assets_hku_logo_horizontal.png")
...
def add_strip(slide, x, y, w, h, title, body, fill, accent):
    add_card(slide, x, y, w, h, fill=fill, shadow=False)
    add_shape(slide, MSO_SHAPE.ROUNDED_RECTANGLE, x + 0.18, y + 0.15, 0.38, h - 0.3, accent, None)
    add_textbox(slide, x + 0.72, y + 0.12, w - 0.9, 0.26, title, size=15.5, color="ink", bold=True, font="Aptos Display")
    add_textbox(slide, x + 0.72, y + 0.36, w - 0.9, h - 0.42, body, size=14.2, color="ink2")


def add_compare_rows(slide, x, y, w, rows, accent, scale_max):
    label_w = 1.45
    bar_w = w - label_w - 1.0
    row_gap = 0.58
    for idx, (label, value, color_name) in enumerate(rows):
        cy = y + idx * row_gap
        add_textbox(slide, x, cy - 0.02, label_w, 0.24, label, size=12.5, color="muted", bold=True)
        add_shape(slide, MSO_SHAPE.ROUNDED_RECTANGLE, x + label_w + 0.1, cy + 0.04, bar_w, 0.16, "sand2", None)
        width = max(0.2, bar_w * (value / scale_max))
        add_shape(slide, MSO_SHAPE.ROUNDED_RECTANGLE, x + label_w + 0.1, cy + 0.04, width, 0.16, color_name or accent, None)
        add_textbox(slide, x + label_w + bar_w + 0.18, cy - 0.04, 0.65, 0.24, f"{value:.2f}", size=12.5, color="ink2", bold=True, align="right")


def add_check_row(slide, x, y, w, text, fill):
    add_shape(slide, MSO_SHAPE.OVAL, x, y + 0.03, 0.26, 0.26, fill, None)
    add_textbox(slide, x + 0.36, y, w - 0.36, 0.34, text, size=15.5, color="ink2", bold=True)
...
def build():
    ...
    if LOGO_PATH.exists():
        slide.shapes.add_picture(str(LOGO_PATH), Inches(22.15), Inches(1.72), width=Inches(8.8))
    add_textbox(slide, 21.9, 3.55, 9.7, 0.48, "Group ID: ________", size=17, color="ink", bold=True)
    add_textbox(slide, 21.9, 3.95, 9.7, 0.78, "Names: ________ / ________ / ________ / ________", size=16.2, color="ink2")
    add_textbox(slide, 21.9, 4.45, 9.7, 0.28, "Put your real names here for the final submission.", size=11.5, color="muted")
    ...
    add_textbox(slide, 1.3, 20.0, 9.1, 1.1, "Label Smoothing helps calibration, but can amplify errors on misclassified samples.", size=20.8, color="ink", bold=True, font="Aptos Display")
    add_strip(slide, 1.32, 21.28, 9.0, 0.8, "Wrong case", "pushes down the true class", "sand2", "orange")
    add_strip(slide, 1.32, 22.18, 9.0, 0.8, "Training signal", "does not directly suppress the false top-1", "paper", "orange")
    add_strip(slide, 1.32, 23.08, 9.0, 0.8, "Representation", "tighter clusters, less class variation", "sand2", "orange")
    add_textbox(slide, 1.34, 24.02, 8.9, 0.6, "The failure mode is specific and memorable: wrong prediction + wrong regularization target.", size=14.8, color="muted", bold=True)

    add_card(slide, 11.6, 19.1, 9.85, 6.0, fill="paper")
    add_pill(slide, 11.9, 19.4, 1.8, 0.42, "METHOD", "blue")
    add_textbox(slide, 11.9, 20.0, 8.9, 0.62, "One substitution", size=22, color="ink", bold=True, font="Aptos Display")
    add_textbox(slide, 11.9, 20.58, 8.9, 0.62, "z_gt  ->  z_max", size=26, color="blue", bold=True, font="Aptos Display")
    add_textbox(slide, 11.9, 21.28, 8.9, 1.0, "L_LS = alpha (z_gt - mean(z))\nL_MaxSup = alpha (z_max - mean(z))", size=17.2, color="ink2")
    add_small_card(slide, 11.95, 22.52, 4.2, 1.42, "Correct case", "acts like confidence regularization", "sand2")
    add_small_card(slide, 16.55, 22.52, 4.2, 1.42, "Wrong case", "suppresses the dominant wrong logit", "blue_soft")
    add_textbox(slide, 11.92, 24.18, 8.9, 0.45, "Simple change. Consistent signal.", size=15.5, color="muted", bold=True)

    add_card(slide, 22.2, 19.1, 9.9, 6.0, fill="paper")
    add_pill(slide, 22.5, 19.4, 2.15, 0.42, "TAKEAWAY", "teal", "white")
    add_textbox(slide, 22.5, 20.0, 9.0, 1.05, "Suppress the overconfident wrong class, not the ground truth.", size=20.8, color="ink", bold=True, font="Aptos Display")
    add_check_row(slide, 22.6, 21.35, 8.7, "same logic on correct and wrong samples", "teal")
    add_check_row(slide, 22.6, 22.1, 8.7, "richer features for transfer and segmentation", "teal")
    add_check_row(slide, 22.6, 22.85, 8.7, "simple loss swap with low overhead", "teal")
    add_textbox(slide, 22.58, 23.78, 8.85, 0.65, "This is the one-sentence ending people should remember when they walk away.", size=14.8, color="muted")
    ...
    add_compare_rows(
        ...
        29.45,
        7.9,
        [("Base", 74.39, "line"), ("LS", 76.08, "blue_mid"), ("MaxSup", 76.49, "blue")],
        "blue",
        77.0,
    )
    add_textbox(slide, 1.42, 31.38, 8.0, 0.5, "+0.41 over LS on DeiT-Small", size=15.3, color="blue", bold=True)
    ...
    add_compare_rows(
        ...
        29.45,
        7.9,
        [("Base", 42.10, "line"), ("LS", 42.40, "teal_soft"), ("MaxSup", 42.80, "teal")],
        "teal",
        43.0,
    )
    add_textbox(slide, 12.07, 31.38, 8.0, 0.5, "Backbone gains transfer to segmentation", size=15.3, color="teal", bold=True)
    ...
    add_small_card(slide, 22.7, 29.3, 2.8, 1.3, "LS", "background\nshortcut bias", "sand2")
    add_small_card(slide, 25.7, 29.3, 2.8, 1.3, "MaxSup", "object body\nshape cues", "blue_soft")
    add_small_card(slide, 28.7, 29.3, 2.8, 1.3, "Meaning", "clearer visual\nreasoning", "teal_soft")
    add_textbox(slide, 22.7, 31.18, 8.8, 0.72, "Interpretability becomes easier to explain because the model attends to the object itself.", size=14.6, color="muted", bold=True)
