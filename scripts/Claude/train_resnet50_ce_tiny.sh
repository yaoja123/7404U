...
# Stable CE baseline on Tiny-ImageNet using plain torchvision ImageFolder
...
# This wrapper intentionally runs directly so you keep the live progress bar.
...
echo "[plain-tiny] CE wrapper"
echo "  env:    ${CONDA_ENV}"
echo "  output: ${HOME}/project/outputs/plain_ce_tiny"
...
exec conda run --no-capture-output -n "${CONDA_ENV}" python "${HERE}/train_resnet50_tiny_plain.py" \
  ...
  --output-dir "${HOME}/project/outputs/plain_ce_tiny" \
  --experiment-name resnet50_ce_tiny \
  --epochs 90 \
  --batch-size 128 \
  --eval-batch-size 128 \
  --workers 2 \
