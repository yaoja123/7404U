...
# Stable MaxSup run on Tiny-ImageNet using plain torchvision ImageFolder
...
echo "[plain-tiny] MaxSup wrapper"
echo "  env:    ${CONDA_ENV}"
echo "  output: ${HOME}/project/outputs/plain_maxsup_tiny"
...
exec conda run --no-capture-output -n "${CONDA_ENV}" python "${HERE}/train_resnet50_tiny_plain.py" \
  ...
  --output-dir "${HOME}/project/outputs/plain_maxsup_tiny" \
  --experiment-name resnet50_maxsup_tiny \
  --epochs 90 \
  --batch-size 128 \
  --eval-batch-size 128 \
  --workers 2 \
